...
$(document).ready(function() {
    ...
    //reset User Password  via AJAX in admin/users/users.blade.php, check admin/js/custom.js    
    $(document).on('click', '.resetUserPassword', function() { // '.updateUserStatus' is the anchor link <a> CSS class    // This is the same as    $('.updateUserStatus').on('click', function() {
        //var status  = $(this).children('i').attr('status'); // Using HTML Custom Attributes
        
        var user_id = $(this).attr('userpassword_id'); // Using HTML Custom Attributes
        var password_id = $(this).attr('passwordval_id'); // Using HTML Custom Attributes
        var new_password = document.getElementById(password_id).value;
        var new_password_length = new_password.length;
        if (new_password_length >= 6 ){
        
        
        // testing if we are collecting the correct value from the password input reset textbox
        //alert(user_id);
        //alert(password_id);
        //alert(new_password);
        $.ajax({
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}, // X-CSRF-TOKEN: https://laravel.com/docs/9.x/csrf#csrf-x-csrf-token    
            type   : 'post',
            url    : '/admin/reset-user-password', // check the web.php for this route and check the UsersController for the updateUserStatus() method
            data   : {new_password: new_password, user_id: user_id}, // we pass the status and user_id
            success: function(resp) {
        ...
        $(document).on('click', '.resetAdminPassword', function() { // '.updateUserStatus' is the anchor link <a> CSS class    // This is the same as    $('.updateAdminStatus').on('click', function() {
            //var status  = $(this).children('i').attr('status'); // Using HTML Custom Attributes
            
            var admin_id = $(this).attr('adminpassword_id'); // Using HTML Custom Attributes
            var password_id = $(this).attr('passwordval_id'); // Using HTML Custom Attributes
            var new_password = document.getElementById(password_id).value;
            var new_password_length = new_password.length;
            if (new_password_length >= 6 ){
            ...
            $.ajax({
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}, // X-CSRF-TOKEN: https://laravel.com/docs/9.x/csrf#csrf-x-csrf-token    
                type   : 'post',
                url    : '/admin/reset-admin-password', // check the web.php for this route and check the UsersController for the updateUserStatus() method
                data   : {new_password: new_password, admin_id: admin_id}, // we pass the status and user_id
                success: function(resp) {
                    alert('password changed succesfully')
                },
                error  : function() {
